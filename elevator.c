...
void reaproveitar_elevadores()
{
    ...
    {
        auxiliar[i].andar_final = calls[i].andar_final;
        calls[i].andar_final = 0;

        auxiliar[i].andar_atual = calls[i].andar_atual;
        calls[i].andar_atual = 0;

        auxiliar[i].corredor_final = calls[i].corredor_final;
        
        auxiliar[i].corredor_atual = calls[i].corredor_atual;

        auxiliar[i].chegou = calls[i].chegou;
        calls[i].chegou = 0;

        calls[i].andar_de_busca = 0;
        calls[i].corredor_de_busca = 0;
...
void caminhar(int andar_desejado, int andar_atual, int coluna_atual, int indice)
{
    if (andar_desejado > andar_atual)
    {
        if (matriz[andar_atual + 1][coluna_atual] == "[\033[91m⊠\033[0m]" || matriz[andar_atual + 1][coluna_atual] == "[\033[31m⊠\033[0m]")
        {
            ...
            matriz[andar_atual + 1][coluna_atual] = "[\033[31m⊠\033[0m]"; // anda uma casa
            matriz[andar_atual][coluna_atual] = "[ ]";            // libera a casa que ele estava anteriormente
            andar_atual++;
        }
        else
        {
...
void limpar_matriz(){
     for (int i = 0; i < LINHAS; i++)
    {
        for (int j = 0; j < COLUNAS; j++)
        {
            if(matriz[i][j] == "[\033[91m⊠\033[0m]" && matriz[i][j] == "[ ]")
                matriz[i][j] = "[ ]"; // vermelho claro (disponivel)
...
int buscando_pedido()
{
    ...
    if (i_fila > 14 && bandeira == 0){
            terminar_execucao();
            reaproveitar_elevadores();
            bandeira = 1;
    }
    if (i_fila > 12 && bandeira == 1){
        for (int i = 0; i < 15; i++)
            printf("\033[32mO elevador  chegou ao seu destino: Andar: %d pela porta %d \n\033[0m", auxiliar[i].andar_atual, auxiliar[i].corredor_atual);

                for (int i = 0; i < 13; i++){
                    printf("\033[32mO elevador  chegou ao seu destino: Andar: %d pela porta %d \n\033[0m", calls[i].andar_final, calls[i].corredor_final);
                    
                    if(matriz[calls[i].andar_final][calls[i].corredor_final] == "[\033[91m⊠\033[0m]" && calls[i].corredor_final < 2)
                        matriz[calls[i].andar_final][calls[i].corredor_final + 1] = "[\033[91m⊠\033[0m]";

                    else if (matriz[calls[i].andar_final][calls[i].corredor_final] == "[\033[91m⊠\033[0m]" && calls[i].corredor_final < 1)
                        matriz[calls[i].andar_final][calls[i].corredor_final + 1] = "[\033[91m⊠\033[0m]";

                    matriz[calls[i].andar_final][calls[i].corredor_final] = "[\033[91m⊠\033[0m]";
                }
